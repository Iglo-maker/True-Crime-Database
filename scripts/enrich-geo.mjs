// Enriches world-atlas TopoJSON with ISO A2 country codes
import { readFileSync, writeFileSync } from 'fs';

// ISO 3166-1 numeric → alpha-2 mapping (most countries)
const numericToAlpha2 = {
  ...
  '158':'TW','170':'CO','174':'KM','175':'YT','178':'CG','180':'CD','184':'CK','188':'CR',
  ...
  '268':'GE','270':'GM','275':'PS','276':'DE','288':'GH','296':'KI','300':'GR','304':'GL',
  ...
  '516':'NA','520':'NR','524':'NP','528':'NL','530':'AN','533':'AW','540':'NC','548':'VU',
  ...
  '638':'RE','642':'RO','643':'RU','646':'RW','654':'SH','659':'KN','660':'AI','662':'LC',
  '666':'PM','670':'VC','674':'SM','678':'ST','682':'SA','686':'SN','688':'RS','690':'SC',
  '694':'SL','702':'SG','703':'SK','704':'VN','705':'SI','706':'SO','710':'ZA','716':'ZW',
  '720':'YE','724':'ES','728':'SS','729':'SD','732':'EH','740':'SR','744':'SJ','748':'SZ',
  '752':'SE','756':'CH','760':'SY','762':'TJ','764':'TH','768':'TG','772':'TK','776':'TO',
  '780':'TT','784':'AE','788':'TN','792':'TR','795':'TM','796':'TC','798':'TV','800':'UG',
  '804':'UA','807':'MK','818':'EG','826':'GB','831':'GG','833':'IM','834':'TZ','840':'US',
  '854':'BF','858':'UY','860':'UZ','862':'VE','876':'WF','882':'WS','887':'YE','894':'ZM',
  '-99':'XK' // Kosovo
};

// Process both files
for (const file of ['countries-110m.json', 'countries-50m.json']) {
  const path = `public/geo/${file}`;
  const topo = JSON.parse(readFileSync(path, 'utf8'));

  for (const geom of topo.objects.countries.geometries) {
    const id = String(geom.id);
    const alpha2 = numericToAlpha2[id] || '';
    geom.properties.iso_a2 = alpha2;
    geom.properties.ISO_A2 = alpha2;
  }
